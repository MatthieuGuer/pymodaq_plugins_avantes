...
if 'linux' in sys.platform: # Linux will have 'linux' or 'linux2'
    #lib = ctypes.CDLL("/usr/local/lib/libavs.so.0")  --> modify because not the good path
    for path in ['/usr/lib64', '/usr/local/lib', '/usr/local/lib/avantes']:
        try:
            lib = ctypes.CDLL("%s/libavs.so" % path)
        except:
            pass
    ...
    if (ctypes.sizeof(ctypes.c_voidp) == 8): # 64 bit
        ...
        # --------------------------------------------------------------------------
        #lib = ctypes.WinDLL("./avaspecx64.dll") #ok si lancement direct
        #lib = ctypes.WinDLL("c:/Users/eric/local_repository/pymodaq_plugins_avantes/src/pymodaq_plugins_avantes/hardware/avaspecx64.dll") #ligne trop longue !!!
